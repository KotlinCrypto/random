...
import org.kotlincrypto.random.internal.js.cryptoRandFill
...
internal actual fun ByteArray.cryptoRandFill() {
    try {
        ...
        cryptoRandFill(procure = jsCryptoFill)
    } catch (t: Throwable) {
        val name = if (IS_NODE_JS) "randomFillSync" else "getRandomValues"
        throw RandomnessProcurementException("Failed to obtain bytes from [crypto.$name]", t)
    }
}
